...
function ServerSettingsModal({ server, onClose }) {
    ...
    async function handleDelete() {
        ...
        queryClient.setQueryData(['servers'], (old) => old ? old.filter(s => s.id !== server.id) : old);
        queryClient.removeQueries({ queryKey: ['server', server.id] });
        queryClient.removeQueries({ queryKey: ['channels', server.id] });
        queryClient.removeQueries({ queryKey: ['members', server.id] });
        onClose();
        navigate('/@me');
    }
            ...
            <div className="rounded-lg w-full max-w-[80%] z-[302] h-full max-h-[85%] flex bg-background overflow-hidden">
                ...
                <div className="w-[280px] flex-shrink-0 bg-guild-bar overflow-y-auto flex flex-col">
                    <nav className="flex flex-col flex-1 py-4 px-4 gap-1">
                        ...
                        <div className="px-2 pb-2 mb-1 border-b border-border">
                            <span className="text-sm font-bold text-foreground truncate block">{server.name}</span>
                        </div>
                ...
                <div className="flex flex-1 overflow-hidden">
                    <div className="flex-1 overflow-y-auto">
                        <SectionContent section={section} server={server} />
                    </div>
