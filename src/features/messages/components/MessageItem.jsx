...
function MessageItem({message, isGrouped = false, disabled = false}) {
    ...
    return disabled || !isGrouped || message.replyTo ? (
        <div onMouseMove={() => !hovered && setHovered(true)}
            ...
            id={`message-${message.id}`} className={`${disabled ? 'pointer-events-none' : ''} ${replyTo && replyTo.id === message.id ? 'bg-primary/10 border-l-2 border-primary hover:bg-primary/15' : 'hover:bg-muted/50'} relative flex flex-col mt-4 transition-colors duration-300  group`}>
            {message.replyTo && (
